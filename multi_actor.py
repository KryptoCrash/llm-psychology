...
def build_no_participants_prompt_from_record(record):
    choices = record["choices"]
    example_letter = "A"
...
def run_diffmean_oracle(records, sample_count=None):
    ...
    for record_idx, record in enumerate(selected_records):
        ...
        no_participants_prompt = build_no_participants_prompt_from_record(record)
        wrong_act, wrong_idx, wrong_token = assistant_start_activation(
            wrong_prompt, act_layer
        )
        no_participants_act, no_participants_idx, no_participants_token = assistant_start_activation(
            no_participants_prompt, act_layer
        )
        diffs.append(wrong_act - no_participants_act)
        examples.append(
            {
                ...
                "no_participants_assistant_start_index": no_participants_idx,
                "wrong_assistant_start_token": wrong_token,
                "no_participants_assistant_start_token": no_participants_token,
            }
        )
        print(
            f"Collected diffmean sample {record_idx + 1}/{len(selected_records)}: "
            f"wrong={wrong_answer}, baseline=no_participants"
        )
    ...
    result = {
        ...
        "direction": "wrong_social_minus_no_participants",
        "condition_a": "all prior participants give the same wrong answer",
        "condition_b": "no other participants are shown; neutral multiple-choice prompt",
        "examples": examples,
