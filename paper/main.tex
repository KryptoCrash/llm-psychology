...
other agents. We study a controlled setting in which Qwen3-8B first answers
selected MMLU and BBH items correctly without social context, and then sees
repeated incorrect answers from prior ``participants.'' Repetition reliably
changes behavior: at ten participants, parseable-conditioned accuracy falls
...
before the final answer substantially reduces this effect, and question
distillation is a stronger mitigation than a devil's-advocate intervention in
...
stimuli are 100 MMLU and 100 BBH items selected because the model suite answers
them correctly without social context. This design deliberately does not
...
selected from saved question banks for which the model suite could answer
...
participant count and are lower under explanation prompting. The same MMLU
directions do not align cleanly with BBH, which suggests either task-specific
representations, prompt-format confounds, or insufficiently stable directions at
this sample size.
