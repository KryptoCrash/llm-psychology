...
these experiments. We then ask whether this behavioral effect has a simple
internal trace. Difference-of-means directions in the residual stream, comparing
...
layer band. At the layer selected from this analysis, MMLU-derived diffmean
directions are highly aligned across high-participant MMLU conditions and show
weaker, but still positive, alignment with BBH. The activation analysis is
...
This design deliberately does not estimate benchmark accuracy. It isolates
cases where an incorrect social signal can pull the model away from a reachable
answer. We measure accuracy,
...
We then examine Qwen's internal activations. Following the broader line of work
that uses contrastive directions in activation space to expose or steer
high-level model behaviors \citep{panickssery2024caa,haon2025vla}, we construct
...
is therefore an empirical localization and cosine-similarity analysis of a social
...
retries unparseable outputs up to ten times and records the final generation. Accuracy
...
selected items; they condition on the observed generations and do not include
...
describes as majority influence in layers 22--33. Layer-23 cosine similarities
between diffmean directions behave sensibly within the matched MMLU domain:
high-participant MMLU directions are strongly aligned with the MMLU $n=8$ and
$n=10$ references, and explanation prompting lowers the average alignment. The
same MMLU directions align less strongly with BBH, suggesting either
task-specific representations, prompt-format effects, or insufficiently stable
...
cosine analysis compares diffmean directions at a single position and selected
layer, so it can miss token-local mechanisms or more distributed
representations. We also
...
activation-oracle summaries and cosine comparisons are generated by
