...
def get_hf_activation_steering_hook(
    ...
    def hook_fn(module, _input, output):
        ...
        assert (
            positions_tensor.max() < L
        ), f"Position {positions_tensor.max()} >= sequence length {L}"
        ...
        steered_KD = (normed_vectors * norms_K1 * steering_coefficient).to(
            resid_BLD.dtype
        )
...
def create_oracle_input(
    ...
    if forced_model_prefix is not None:
        forced_prefix_ids = tokenizer.encode(
            forced_model_prefix, add_special_tokens=False
        )
        input_prompt_ids = input_prompt_ids + forced_prefix_ids

    # Find ? token positions in the prompt
    positions = find_pattern_in_tokens(
        input_prompt_ids, SPECIAL_TOKEN, num_positions, tokenizer
    )
...
def _run_evaluation(
    ...
    with torch.no_grad():
        all_responses = []
        for oracle_input in tqdm(
            eval_data, desc="Evaluating model", disable=not verbose
        ):
            # Generate response
...
def _create_oracle_inputs(
    ...
    # Full sequence probes (all activations)
        ...
        raise ValueError(
            f"Unknown oracle_input_type: {oracle_input_type}. Must be 'tokens', 'segment', or 'full_seq'"
        )
...
def run_oracle(
    ...
    inputs_BL = tokenizer(
        [target_prompt], return_tensors="pt", add_special_tokens=False, padding=True
    ).to(device)
